...
void GUI::think() {
	...
	// the factor was resolved in Client::OnPaint. if it moved this frame the
	// design-space canvas resized under us.
	if (ui::scale_changed()) {
		// the design-space canvas just resized under us, drag every form back
		// into it so nothing ends up unreachable.
		for (const auto& f : m_forms) {
			if (!f)
				continue;

			math::clamp(f->m_x, 0, std::max(0, ui::logical_w() - f->m_width));
			math::clamp(f->m_y, 0, std::max(0, ui::logical_h() - f->m_height));
		}
	}
	...
	// this is the input half of the scaling boundary - bring the real cursor
	// down into design px so every element rect can stay unscaled.
	g_input.m_mouse.x = ui::unpx(g_input.m_mouse.x);
	g_input.m_mouse.y = ui::unpx(g_input.m_mouse.y);

	// mouse isnt clamped across multiple displays etc.
	math::clamp(g_input.m_mouse.x, 0, ui::logical_w());
	math::clamp(g_input.m_mouse.y, 0, ui::logical_h());
...
bool GUI::IsCapturingKeys() const {
	for (const auto& f : m_forms) {
		if (!f || !f->m_open)
			continue;

		// an edit box has focus, every keystroke belongs to it.
		if (f->m_active_element && f->m_active_element->m_type == ElementTypes::EDIT)
			return true;

		if (!f->m_active_tab)
			continue;

		// a keybind is waiting for its key. keybinds never become the active
		// element ( no ElementFlags::ACTIVE ), so we have to scan the tab.
		for (const auto& e : f->m_active_tab->m_elements) {
			if (!e || e->m_type != ElementTypes::KEYBIND)
				continue;
