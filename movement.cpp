...
void Movement::AutoPeek(CUserCmd* cmd, float wish_yaw) {
	...
	if (g_input.GetKeyState(g_menu.main.aimbot.quickpeekassist.get())) {
		...
		// that should fix the memory leak;
		return;
	}
}
...
void Movement::QuickStop() {
	if (!g_cl.m_pressing_move && g_menu.main.aimbot.quick_stop.get() && g_cl.m_speed > 15.f) {
...
{
	if (!g_input.GetKeyState(g_menu.main.misc.edgejump.get()))
		return;

	// get local player.
	Player* player = (Player*)(g_csgo.m_entlist->GetClientEntity(g_csgo.m_engine->GetLocalPlayer()));
	...
	if (g_cl.m_flags & FL_ONGROUND && !(player->m_fFlags() & FL_ONGROUND) && g_input.GetKeyState(g_menu.main.misc.edgejump.get())) {
		cmd->m_buttons |= IN_JUMP; // do edge jump.
	}

	if (!(g_cl.m_local->m_fFlags() & FL_ONGROUND) &&
		(g_cl.m_flags & FL_ONGROUND))
