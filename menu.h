...
public:
	void init() {
		...
		hitbox.setup(XOR("Target hitboxes"), XOR("hitbox"), { XOR("Head"), XOR("Chest"), XOR("Stomach"), XOR("Arms"), XOR("Legs"), XOR("Feet")});
		RegisterElement(&hitbox);

		multipoint.setup(XOR("Multi-point"), XOR("multipoint"), { XOR("Head"), XOR("Chest"), XOR("Stomach"), XOR("Arms"), XOR("Legs"), XOR("Feet")});
		RegisterElement(&multipoint);

		head_scale.setup("", XOR("head_scale"), 1.f, 100.f, false, 0, 55.f, 1.f, XOR(L"%"));
		head_scale.AddShowCallback(callbacks::IsMultipointOn);
		RegisterElement(&head_scale);

		body_scale.setup(XOR("Stomach hitbox scale"), XOR("body_hitbox_scale"), 1.f, 100.f, true, 0, 80.f, 1.f, XOR(L"%"));
		body_scale.AddShowCallback(callbacks::IsMultipointBodyOn);
		RegisterElement(&body_scale);

		ignore_limbs.setup(XOR("Ignore limbs if moving"), XOR("ignore_limbs"));
		ignore_limbs.AddShowCallback(callbacks::IsLimbsOn);
		RegisterElement(&ignore_limbs);

		silent.setup(XOR("Silent aim"), XOR("silent"));
		RegisterElement(&silent);

		hitchance_amount.setup("Minimum hitchance", XOR("hitchance_amount"), 0.f, 100.f, true, 0, 50.f, 1.f, XOR(L"%"));
		...
		hitchance_in_air.setup(XOR("Override hitchance (in-air)"), XOR("custom_in_air_hitchance"));
		hitchance_in_air.AddShowCallback(callbacks::IsHitchanceOn);
		RegisterElement(&hitchance_in_air);

		in_air_hitchance.setup("", XOR("in_air_hitchance"), 1.f, 100.f, false, 0, 50.f, 1.f, XOR(L"%"));
		...
		minimal_damage.setup(XOR("Minimum damage"), XOR("minimal_damage"), 0.f, 126.f, true, 0, 40.f, 1.f, XOR(L"HP"));
		...
		override_dmg_value.setup(" ", XOR("dmg_override"), 0.f, 126.f, false, 0, 20.f, 1.f, XOR(L"HP"));
		...
		log_spread.setup(XOR("Log misses due to spread"), XOR("log_spread"), true, true);
		RegisterElement(&log_spread);

		// col2.
		nospread.setup(XOR("Remove spread"), XOR("nospread"), true, false, false);
		RegisterElement(&nospread, 1);

		norecoil.setup(XOR("Remove recoil"), XOR("norecoil"));
		RegisterElement(&norecoil, 1);

		correct.setup(XOR("Anti-aim corrections"), XOR("correct"));
		RegisterElement(&correct, 1);

		correct_opt.setup(XOR(""), XOR("correct_opt"), { XOR("Anti-aim resolver"), XOR("Extended backtrack") }, false);
		...
		quick_stop_mode.setup(XOR(""), XOR("quick_stop_mode"), { "Fakewalk", "Move between shots", "Stop", "In air", "Early" }, false);
		...
		delay_shot_center.setup(XOR("Delay shot"), XOR("delay_shot_center"), { XOR("Off"), XOR("Default"), XOR("High") });
		RegisterElement(&delay_shot_center, 1);

		prefer_accuracy.setup(XOR("Prefer delay shot"), XOR("prefer_accuracy"), { XOR("Default"), XOR("On tick") });
		RegisterElement(&prefer_accuracy, 1);

		prefer_baim.setup(XOR("Prefer body-aim"), XOR("prefer_baim"));
		RegisterElement(&prefer_baim, 1);

		prefer_baim_disablers.setup(XOR("Prefer body-aim disablers"), XOR("prefer_baim_disablers"), { XOR("LBY update"), XOR("Target moving"), XOR("Target resolved"), XOR("Low body yaw") });
		prefer_baim_disablers.AddShowCallback(callbacks::IsPFBOn);
		RegisterElement(&prefer_baim_disablers, 1);

		debugaim.setup(XOR("Draw aimbot records"), XOR("debugaim"));
		RegisterElement(&debugaim, 1);

		debugaim_color.setup(XOR("Color"), XOR("shot_record_color"), colors::white);
		debugaim_color.AddShowCallback(callbacks::IsDebugOn);
		RegisterElement(&debugaim_color, 1);

		debugaim_mode.setup(XOR(""), XOR("matrix_type"), { XOR("Hitbox"), XOR("Skeleton"), XOR("Hitbox (on-hit)"), XOR("Hitbox + overlay") }, false);
		debugaim_mode.AddShowCallback(callbacks::IsDebugOn);
		RegisterElement(&debugaim_mode, 1);

		debuglog.setup(XOR("Log records"), XOR("debuglog"));
		debuglog.AddShowCallback(callbacks::IsDebugOn);
		...
		override.setup(XOR("Anti-aim resolver override"), XOR("override on key"));
		RegisterElement(&override, 1);

		doubletap.setup(XOR("Double tap"), XOR("doubletap"), false);
...
public:
	...
	Checkbox	  manual_enabled;
	MultiDropdown manual_ignore;
	Keybind		  manual_left;
	Keybind		  manual_right;
	Keybind		  manual_back;
	Keybind		  manual_front;
	Checkbox	  fakelag_chams;
	Colorpicker	  fakelag_clr;
...
public:
	void init() {
		...
		mode.setup("Condition", XOR("mode"), { XOR("Standing"), XOR("Walking"), XOR("Air") }, true);
		RegisterElement(&mode);

		at_targets.setup("Yaw base", XOR("at_targets"), { XOR("Local view"), XOR("Away from crosshair"), XOR("Nearest distance") }, false );
		...
		dir_stand.setup(XOR("Direction"), XOR("dir_stnd"), { XOR("Automatic"), XOR("Backwards"), XOR("Left"), XOR("Right"), XOR("Custom") });
		...
		dir_walk.setup(XOR("Direction"), XOR("dir_walk"), { XOR("Automatic"), XOR("Backwards"), XOR("Left"), XOR("Right"), XOR("Custom") });
		...
		dir_air.setup(XOR("Direction"), XOR("dir_air"), { XOR("Automatic"), XOR("Backwards"), XOR("Left"), XOR("Right"), XOR("Custom") });
		...
		// col 2.
		fakelag_enable.setup(XOR("Fake-latency"), XOR("lag_enable"));
		RegisterElement(&fakelag_enable, 1);

		fakelag_conditions.setup("", XOR("lag_active"), { XOR("Standing"), XOR("While moving"), XOR("On air"), XOR("While crouching"), XOR("On lby update") }, false);
		RegisterElement(&fakelag_conditions, 1);

		fakelag_mode.setup("", XOR("lag_mode"), { XOR("Maximum"), XOR("Dynamic"), XOR("Random") }, false);
		...
		fakelag_reset_bhop.setup(XOR("Reset on bunnyhop"), XOR("lag_land"), true, false, false);
		RegisterElement(&fakelag_reset_bhop, 1);

		fakelag_fluct.setup(XOR("Fluctuation"), XOR("lag_fluct"));
		RegisterElement(&fakelag_fluct, 1);

		fakelag_fluct_amt.setup(XOR(""), XOR("lag_fluctuate_amt"), 20.f, 48.f, false, 0, 0.f, 1.f, L"t");
		fakelag_fluct_amt.AddShowCallback(callbacks::IsFluctuateOn);
		RegisterElement(&fakelag_fluct_amt, 1);

		fakelag_fluct_ticks.setup(XOR("Limit"), XOR("lag_ticks_to_hold"), 1.f, 16.f);
		...
		manual_enabled.setup(XOR("Enable manual anti-aim"), XOR("manual_enabled"));
		RegisterElement(&manual_enabled, 1);

		manual_ignore.setup(XOR("Manual disablers"), XOR("manual_ignore"), { "Ignore jitter", "Ignore rotate" });
		manual_ignore.AddShowCallback(callbacks::IsManualOn);
		RegisterElement(&manual_ignore, 1);

		manual_left.setup(XOR("Left"), XOR("manual_left"));
		manual_left.AddShowCallback(callbacks::IsManualOn);
		manual_left.SetToggleCallback(callbacks::ToggleLeft);
		RegisterElement(&manual_left, 1);

		manual_right.setup(XOR("Right"), XOR("manual_right"));
		manual_right.AddShowCallback(callbacks::IsManualOn);
		manual_right.SetToggleCallback(callbacks::ToggleRight);
		RegisterElement(&manual_right, 1);

		manual_back.setup(XOR("Backwards"), XOR("manual_back"));
		manual_back.AddShowCallback(callbacks::IsManualOn);
		manual_back.SetToggleCallback(callbacks::ToggleBack);
		RegisterElement(&manual_back, 1);

		manual_front.setup(XOR("Forward"), XOR("manual_front"));
		manual_front.AddShowCallback(callbacks::IsManualOn);
...
public:
	void init() {
		...
		hitmarker_select.setup(XOR("Hitmarker"), XOR("hitmarker_select"), { XOR("Off"), XOR("Crosshair"), XOR("World") });
		RegisterElement(&hitmarker_select);
		...
		offscreen_size.setup(XOR(""), XOR("offscreen_size"), 2.f, 32.f, false, 0, 18.f, 1.f, XOR(L"px"));
		offscreen_size.AddShowCallback(callbacks::outofpov);
		RegisterElement(&offscreen_size);

		offscreen_distance.setup(XOR(""), XOR("offscreen_distance"), 10.f, 100.f, false, 0, 200.f, 1.f, XOR(L"%"));
		...
		chams_local.setup(XOR("Enabled"), XOR("chams_local"));
		chams_local.AddShowCallback(callbacks::IsChamsSelection0);
		RegisterElement(&chams_local, 1);

		chams_local_mat.setup(XOR("Type"), XOR("chams_local_mat"), { XOR("Default"), XOR("Solid"), XOR("Metallic"), XOR("Rim"), XOR("Glow") });
		...
		chams_local_scope_blend.setup("", XOR("chams_local_scope_blend"), 0.f, 100.f, false, 0, 15.f, 1.f, XOR(L"%"));
		chams_local_scope_blend.AddShowCallback(callbacks::IsChamsSelection0);
		RegisterElement(&chams_local_scope_blend, 1);

		chams_enemy.setup(XOR("Enabled"), XOR("chams_enemy"), { XOR("Player"), XOR("Player (behind wall)") });
		chams_enemy.AddShowCallback(callbacks::IsChamsSelection1);
		RegisterElement(&chams_enemy, 1);

		chams_enemy_mat.setup(XOR("Type"), XOR("chams_enemy_mat"), { XOR("Default"), XOR("Solid"), XOR("Metallic"), XOR("Rim"), XOR("Glow"), XOR("Bubble") });
		...
		chams_enemy_invis.setup(XOR("Color behind wall"), XOR("chams_enemy_invis"), { 60, 120, 180 });
		chams_enemy_invis.AddShowCallback(callbacks::IsChamsSelection1);
		RegisterElement(&chams_enemy_invis, 1);

		chams_enemy2_invis.setup(XOR("Glow color (behind wall)"), XOR("chams_enemy2_invis"), { 255, 255, 200, 180 });
		chams_enemy2_invis.AddShowCallback(callbacks::EnemyGlowChamsSelection);
		RegisterElement(&chams_enemy2_invis, 1);

		chams_enemy_blend.setup("Visible alpha", XOR("chams_enemy_blend"), 0.f, 100.f, true, 0, 100.f, 1.f, XOR(L"%"));
		chams_enemy_blend.AddShowCallback(callbacks::IsChamsSelection1);
		RegisterElement(&chams_enemy_blend, 1);

		chams_enemy_blend2.setup("Behind wall alpha", XOR("chams_enemy_blend2"), 0.f, 100.f, true, 0, 85.f, 1.f, XOR(L"%"));
		chams_enemy_blend2.AddShowCallback(callbacks::IsChamsSelection1);
		RegisterElement(&chams_enemy_blend2, 1);

		chams_enemy_reflectivity.setup(XOR("Metallic color"), XOR("chams_enemy_reflectivity"), { 255,178,00 });
		chams_enemy_reflectivity.AddShowCallback(callbacks::EnemyMetallicChamsSelection);
		RegisterElement(&chams_enemy_reflectivity, 1);

		chams_reflectivity.setup("Reflectivity", XOR("chams_reflectivity"), 0.f, 100.f, true, 0, 67.f, 1.f, XOR(L"%"));
		chams_reflectivity.AddShowCallback(callbacks::EnemyMetallicChamsSelection);
		RegisterElement(&chams_reflectivity, 1);

		chams_enemy_shine.setup("Shine", XOR("chams_enemy_shine"), 0.f, 100.f, true, 0, 10.f, 1.f, XOR(L"%"));
		chams_enemy_shine.AddShowCallback(callbacks::EnemyMetallicChamsSelection);
		RegisterElement(&chams_enemy_shine, 1);

		chams_enemy_rim.setup("Rim", XOR("chams_enemy_rim"), 0.f, 100.f, true, 0, 10.f, 1.f, XOR(L"%"));
		chams_enemy_rim.AddShowCallback(callbacks::EnemyMetallicChamsSelection);
		RegisterElement(&chams_enemy_rim, 1);

		chams_ragdoll.setup("Ragdoll", XOR("chams_ragdoll"));
		chams_ragdoll.AddShowCallback(callbacks::IsChamsSelection1);
		RegisterElement(&chams_ragdoll, 1);

		chams_friendly.setup(XOR("Enabled"), XOR("chams_friendly"), { XOR("Teammate"), XOR("Teammate (behind wall)") });
		chams_friendly.AddShowCallback(callbacks::IsChamsSelection2);
		RegisterElement(&chams_friendly, 1);

		chams_friendly_mat.setup(XOR("Type"), XOR("chams_friendly_mat"), { XOR("Default"), XOR("Solid"), XOR("Metallic"), XOR("Rim"), XOR("Glow"), XOR("Bubble") });
		...
		chams_friendly_invis.setup(XOR("Color behind wall"), XOR("chams_friendly_invis"), { 255, 50, 0 });
		...
		chams_enemy_history.setup(XOR("Enabled"), XOR("chams_history"));
		chams_enemy_history.AddShowCallback(callbacks::IsChamsSelection3);
		RegisterElement(&chams_enemy_history, 1);

		chams_enemy_history_mat.setup(XOR("Type"), XOR("chams_enemy_history_mat"), { XOR("Default"), XOR("Solid"), XOR("Metallic"), XOR("Rim"), XOR("Glow"), XOR("Bubble") });
		...
		history_skeleton.setup(XOR("Draw skeleton"), XOR("history_skeleton"));
		history_skeleton.AddShowCallback(callbacks::IsChamsSelection3);
		RegisterElement(&history_skeleton, 1);

		history_skeleton_col.setup(XOR("Color"), XOR("history_skeleton_col"), { 162, 162, 162 });
		history_skeleton_col.AddShowCallback(callbacks::IsChamsSelection3);
		RegisterElement(&history_skeleton_col, 1);

		chams_fake.setup(XOR("Enabled"), XOR("chams_fake"));
		chams_fake.AddShowCallback(callbacks::IsChamsSelection4);
		RegisterElement(&chams_fake, 1);

		chams_fake_mat.setup(XOR("Type"), XOR("chams_fake_mat"), { XOR("Default"), XOR("Solid"), XOR("Metallic"), XOR("Rim"), XOR("Glow"), XOR("Bubble") });
		...
		chams_shot.setup(XOR("Enabled"), XOR("chams_shot"));
		chams_shot.AddShowCallback(callbacks::IsChamsSelection5);
		RegisterElement(&chams_shot, 1);

		chams_shot_mat.setup(XOR("Type"), XOR("chams_shot_mat"), { XOR("Default"), XOR("Solid"), XOR("Metallic"), XOR("Rim"), XOR("Glow"), XOR("Bubble") });
		...
		chams_shot_fadetime.setup("Duration", XOR("chams_shot_fadetime"), 0.0f, 10, true, 1, 0.3f, 0.1f, XOR(L"s"));
		chams_shot_fadetime.AddShowCallback(callbacks::IsChamsSelection5);
		RegisterElement(&chams_shot_fadetime, 1);

		weapon_chams.setup(XOR("Enabled"), XOR("weapon_chams"));
		weapon_chams.AddShowCallback(callbacks::IsChamsSelection6);
		RegisterElement(&weapon_chams, 1);

		weapon_chams_mat.setup(XOR("Type"), XOR("weapon_chams_mat"), { XOR("Default"), XOR("Solid"), XOR("Metallic"), XOR("Rim"), XOR("Glow"), XOR("Bubble") });
		weapon_chams_mat.AddShowCallback(callbacks::IsChamsSelection6);
		RegisterElement(&weapon_chams_mat, 1);

		weapon_chams_col.setup(XOR("Color"), XOR("weapon_chams_col"), { 0,0,0 });
		...
		weapon_chams_col2.setup(XOR("Additive"), XOR("weapon_chams_col2"), { 255, 255, 255 });
...
public:
	...
	Colorpicker   molotov_radius;
	Colorpicker   smoke_radius;
	Slider		  molotov_col_alpha;
	Slider	      smoke_col_alpha;
	Checkbox	  planted_c4;
	Colorpicker   c4_col;
	Slider	      c4_blend;
	...
	Slider   impact_duration;

	Checkbox	antiaim_arrows;
	Checkbox	manual_anti_aim_indic;
	Colorpicker manual_anti_aim_col;

	Checkbox	remove_skybox;
	Checkbox	grenade_warning;

public:
	void init( ) {
		SetTitle( XOR( "Effects" ) );

		enemy_radar.setup(XOR("Radar"), XOR("enemy_radar"));
		RegisterElement(&enemy_radar);

		dropped_weapons.setup(XOR("Dropped weapons"), XOR("dropped_weapons"), { XOR("Text"), XOR("Distance"), XOR("Ammo"), XOR("Glow") });
		...
		item_color_alpha.setup("", XOR("item_color_alpha"), 0.f, 255, false, 0, 180, 1.f);
		item_color_alpha.AddShowCallback(callbacks::droppedwpnson);
		RegisterElement(&item_color_alpha);

		glow_color_alpha.setup("Glow alpha", XOR("glow_color_alpha"), 0.f, 100.f, true, 0, 85.f, 1.f, XOR(L"%"));
		glow_color_alpha.AddShowCallback(callbacks::droppedwpnsglow);
		RegisterElement(&glow_color_alpha);

		proj.setup(XOR("Grenades"), XOR("proj"));
		RegisterElement(&proj);

		proj_color.setup(XOR("Grenade color"), XOR("proj_color"), colors::burgundy);
		...
		proj_col_slider.AddShowCallback(callbacks::IsProjectiles);
		RegisterElement(&proj_col_slider);

		molotov_radius.setup(XOR("Molotov radius"), XOR("molotov_radius"), { 247, 180, 22 });
		molotov_radius.AddShowCallback(callbacks::IsProjectiles);
		RegisterElement(&molotov_radius);

		smoke_radius.setup(XOR("Smoke radius"), XOR("smoke_radius"), { 20, 100, 250 });
		smoke_radius.AddShowCallback(callbacks::IsProjectiles);
		RegisterElement(&smoke_radius);
		...
		c4_col.setup(XOR("Bomb color"), XOR("c4_col"), { 150, 200, 60 });
		c4_col.AddShowCallback(callbacks::bombon);
		RegisterElement(&c4_col);

		c4_blend.setup(XOR(""), XOR("c4_blend"), 0.f, 100.f, false, 0, 75, 1.f, XOR(L"%"));
		...
		manual_anti_aim_indic.setup(XOR("Manual indicators"), XOR("manual_antiaim"));
		RegisterElement(&manual_anti_aim_indic, 1);

		manual_anti_aim_col.setup(XOR("Color"), XOR("manual_anti_aim_col"), colors::burgundy);
		manual_anti_aim_col.AddShowCallback(callbacks::manualaaindon);
		...
		RegisterElement(&remove_skybox, 1);
		...
		transparent_props_amount.setup("Transparent props", XOR("transparent_props_amount"), 0.f, 100.f, true, 0, 100, 1.f, XOR(L"%"));
		transparent_props_amount.SetCallback(Visuals::ModulateWorld);
		RegisterElement(&transparent_props_amount, 1);

		world.setup(XOR("Brightness adjustment"), XOR("world"), { XOR("Nightmode"), XOR("Fullbright")});
		...
		thirdperson_distance.setup(XOR("Distance"), XOR("thirdperson_distance"), 100.f, 200.f, true, 0.f, 120.f, 1.f, XOR(L"in"));
		//thirdperson_distance.AddShowCallback(callbacks::IsTPON);
		...
		impact_duration.setup(XOR(""), XOR("impact_duration"), 2, 10, false, 0, 4.f, 1.f, XOR(L"s"));
		impact_duration.AddShowCallback(callbacks::bulletimpacts_on);
		RegisterElement(&impact_duration, 1);
	}
};
...
public:
	void init( ) {
		SetTitle( XOR( "Skinchanger" ) );

		enable.setup( XOR( "Enabled" ), XOR( "skins_enable" ) );
		...
		// col 2.
		knife.setup( XOR( "Override knife" ), XOR( "skins_knife_model" ), { XOR( "Off" ), XOR( "Bayonet" ), XOR( "Bowie" ), XOR( "Butterfly" ), XOR( "Falchion" ), XOR( "Flip" ), XOR( "Gut" ), XOR( "Huntsman" ), XOR( "Karambit" ), XOR( "M9 bayonet" ), XOR( "Shadow daggers" ) } );
		knife.SetCallback( callbacks::SkinUpdate );
		RegisterElement( &knife, 1 );

		glove.setup( XOR( "Override gloves" ), XOR( "skins_glove_model" ), { XOR( "Off" ), XOR( "Bloodhound" ), XOR( "Sport" ), XOR( "Driver" ), XOR( "Handwraps" ), XOR( "Motorcycle" ), XOR( "Specialist" ) } );
		glove.SetCallback( callbacks::ForceFullUpdate );
		RegisterElement( &glove, 1 );

		glove_id.setup( XOR( "Glove ID" ), XOR( "skins_glove_id" ), 2 );
...
public:
	void init( ) {
		...
		RegisterElement(&clantag);

		edgejump.setup(XOR("Jump at edge"), XOR("edgejump"), true, 0x45); // e
		//edgejump.AddShowCallback(callbacks::ToggleEdgeJump);
		RegisterElement(&edgejump);
		...
		secondary_fake_latency.setup(XOR("Ping spike (alternative)"), XOR("secondary_fake_latency"));
		...
		whitelist.setup(XOR("Ignore other users"), XOR("whitelist"));
		...
		motion_blur.setup(XOR("Motion blur"), XOR("motion_blur"), 0.f, 100.f, true, 0, 0.f, 1.f, XOR(L"x"));
		RegisterElement(&motion_blur, 1);	

		aspect.setup(XOR("Force aspect ratio"), XOR("aspect"), 0.40f, 2, true, 2, 1.78f, 0.01f);
		RegisterElement(&aspect, 1);

		config.setup(XOR("Presets"), XOR("cfg"), { XOR("1"), XOR("2"), XOR("3"), XOR("4"), XOR("5"), XOR("6") });
		config.RemoveFlags(ElementFlags::SAVE);
		RegisterElement(&config, 1);

		key1.setup(XOR("Configuration key"), XOR("cfg_key1"));
		...
		key2.setup(XOR("Configuration key"), XOR("cfg_key2"));
		...
		key3.setup(XOR("Configuration key"), XOR("cfg_key3"));
		...
		key4.setup(XOR("Configuration key"), XOR("cfg_key4"));
		...
		key5.setup(XOR("Configuration key"), XOR("cfg_key5"));
		...
		key6.setup(XOR("Configuration key"), XOR("cfg_key6"));
