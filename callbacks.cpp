...
bool callbacks::droppedwpnson() {
	return !g_menu.main.visuals.dropped_weapons.GetActiveIndices().empty();
}

bool callbacks::droppedwpnsglow() {
	return g_menu.main.visuals.dropped_weapons.get(3);
}
...
bool callbacks::manualaaindon() {
	return g_menu.main.visuals.manual_anti_aim_col.get();
}
...
bool callbacks::IsDoubleTap() {
	return g_menu.main.aimbot.doubletap.get() != -1;
}
...
bool callbacks::IsDebugOn() {
	return g_menu.main.aimbot.debugaim.get();
}

bool callbacks::IsManualOn() {
	return g_menu.main.antiaim.manual_enabled.get();
}
...
bool callbacks::bulletimpacts_on() {
...
bool callbacks::IsHitchanceOn( ) {
	return g_menu.main.aimbot.hitchance_amount.get( ) > 0;
}

bool callbacks::IsHitmarker( ) {
	return g_menu.main.players.hitmarker_select.get() != 0;
}
...
bool callbacks::IsLimbsOn() {
	return (g_menu.main.aimbot.hitbox.get(3) || g_menu.main.aimbot.hitbox.get(5));
}
