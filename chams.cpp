...
void Chams::RenderPlayer(Player* player) {
	...
	if (player->m_bIsLocalPlayer()) {
		...
		if (type == WEAPONTYPE_C4 || type == WEAPONTYPE_GRENADE || player->m_bIsScoped())
			SetAlpha(0.2f);
	...
	if (player->GetClientClass()->m_ClassID == g_netvars.GetClientID(HASH("CCSRagdoll")) && enemy && g_menu.main.players.chams_ragdoll.get()) {
