...
void AimPlayer::SetupHitboxes(LagRecord* record, bool history) {
	...
	bool ignore_limbs = record->m_velocity.length_2d() > 71.f && g_menu.main.aimbot.ignore_limbs.get();
	bool disable_feet = g_menu.main.aimbot.optimizations.get(4);
	...
	// feet.
	if (hitbox.get(5) && !ignore_limbs && !disable_feet) {
...
void Aimbot::FinishTargetSelection() {
	...
	while (this->m_targets.size() > targets)
...
void Aimbot::find() {
	...
	if (found_target) {
		...
		bool on = (g_menu.main.aimbot.hitchance_amount.get() > 0) && !g_menu.main.aimbot.nospread.get();
...
bool Aimbot::CheckHitchance(Player* player, int hitbox, const ang_t& angle) {
	...
	const int   SEED_MAX = g_menu.main.aimbot.optimizations.get(1) ? 111 : 255;
...
bool AimPlayer::SetupHitboxPoints(LagRecord* record, BoneArray* bones, int index, std::vector< vec3_t >& points) {
	...
	if (bbox->m_radius <= 0.f) {
		...
		if (index == HITBOX_R_FOOT || index == HITBOX_L_FOOT) {
			...
			if (g_menu.main.aimbot.multipoint.get(5) && !g_menu.main.aimbot.optimizations.get(3)) {
	...
	// these hitboxes are capsules.
		...
		// arms get only one point.
			...
			// elbow.
			if (g_menu.main.aimbot.multipoint.get(3) && !g_menu.main.aimbot.optimizations.get(2))
...
float GetMinDamage(int weaponIndex) {

	// doing these for important weapons only, will set a global value for the rest lol
	switch (weaponIndex) {
	case DEAGLE:
		return 15.f;
	case ELITE:
	...
	default:
		return 8.f; // default value for the rest of the guns
	}
}

// gamesense automatic damage calculations here
int GetScaledMinDamage(int originalDamage, int enemyHealth) {
	float scale = static_cast<float>(enemyHealth) / 100.0f;
	int scaledDamage = std::round(originalDamage * scale);
	return std::max(scaledDamage, 1);
}
...
bool AimPlayer::GetBestAimPosition(vec3_t& aim, float& damage, int& hitbox, int& hitgroup, LagRecord* record) {
	...
	if (g_cl.m_weapon_id == ZEUS) {
		...
		else if (dmg == 0) { // automatic min damage here
			float get_min_dmg = GetMinDamage(g_cl.m_weapon_id);
			float scaled_dmg = GetScaledMinDamage(get_min_dmg, hp);

			dmg = pendmg = scaled_dmg;
			pen = true;
		}
