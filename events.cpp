...
void events::player_hurt( IGameEvent* evt ) {
	...
	if (evt) {
		...
		for (int i = 0; i < g_visuals.m_impacts.size(); i++) {
			...
			vec3_t position = iter.m_impact_pos;
...
void events::bullet_impact(IGameEvent* evt) {
	...
	g_visuals.m_impact_pos = pos;
	g_visuals.m_impacts.push_back({pos, g_cl.m_local->GetShootPosition(), g_cl.m_local->m_nTickBase()});

	// draw our (server-side) bullet impacts.
	if (g_menu.main.visuals.bullet_impacts.get())
		g_csgo.m_debug_overlay->AddBoxOverlay(pos, vec3_t(-2, -2, -2), vec3_t(2, 2, 2), ang_t(0, 0, 0), 0, 0, 255, 127, g_menu.main.visuals.impact_duration.get());
}
...
void events::bomb_beginplant( IGameEvent* evt ) {
	...
	nSite = g_csgo.m_entlist->GetClientEntity(evt->m_keys->FindKey(HASH("site"))->GetInt());
}
